...
export default function App() {
    ...
    const handleEndSet = () => {
        ...
        setLineup({ p1: null, p2: null, p3: null, p4: null, p5: null, p6: null }); setLiberos([]); setSetterId(null); setBench([]); setPointLog([]); setHistory([]); setSetupStep('players'); setMatchPhase('lineup_setup'); setModal(null); setLiberoServingFor(null); setLiberoHasServedFor(null);
    };
    ...
    const incrementStats = (stats, playerId, statToLog, currentSetterId) => {
        const newStats = JSON.parse(JSON.stringify(stats));
        const increment = (pId, s, value = 1) => { if (!newStats[pId]) newStats[pId] = {}; newStats[pId][s] = (newStats[pId][s] || 0) + value; };
    ...
    const assignStatToPlayer = (playerId, stat) => {
        ...
        switch(statToLog) {
            ...
            case 'Serve Error': case 'Set Error': case 'RE': case 'Block Error': pointWinner = 'opponent'; logMessage = `O: ${statToLog} by #${player.number} ${player.name}`; break;
        }
    ...
    const handleKwdaSelection = (attackerId) => {
        ...
        const updateKwdaStats = (stats) => { const newStats = JSON.parse(JSON.stringify(stats)); const increment = (pId, s) => { if (!newStats[pId]) newStats[pId] = {}; newStats[pId][s] = (newStats[pId][s] || 0) + 1; }; increment(attackerId, 'Kill'); increment(attackerId, 'Hit Attempt'); return newStats; };
    ...
    const handleCourtClickForLineup = (position) => {
        if (setupStep === 'players') {
            ...
            else { const lineupIsFullBeforeRemoval = Object.values(lineup).every(p => p !== null); if (lineupIsFullBeforeRemoval) { setSetupStep('players'); } setLineup(prev => ({ ...prev, [position]: null })); }
    ...
    const LineupSetup = () => {
        ...
        const handleToggleLibero = (playerId) => { const newSelection = new Set(tempLiberos); if (newSelection.has(playerId)) { newSelection.delete(playerId); } else { if (newSelection.size < 2) { newSelection.add(playerId); } } setTempLiberos(newSelection); };
    ...
    const RosterModal = () => { const [localRoster, setLocalRoster] = useState([{ number: '', name: '' }]); const [localMatchName, setLocalMatchName] = useState(''); const [localHomeTeam, setLocalHomeTeam] = useState(''); const [localOpponentTeam, setLocalOpponentTeam] = useState(''); const addPlayer = () => setLocalRoster([...localRoster, { number: '', name: '' }]); const updatePlayer = (index, field, value) => { if (field === 'number' && value && parseInt(value, 10) < 0) return; const newRoster = [...localRoster]; newRoster[index][field] = value; setLocalRoster(newRoster); }; const handleSubmit = (e) => { e.preventDefault(); const filtered = localRoster.filter(p => p.number && p.name); if (filtered.length < 6) { alert("Please enter at least 6 players."); return; } handleSaveRoster(filtered, localMatchName, localHomeTeam, localOpponentTeam); }; return (<form onSubmit={handleSubmit}> <div className="grid grid-cols-2 gap-4 mb-4"> <input type="text" placeholder="Home Team Name" value={localHomeTeam} onChange={(e) => setLocalHomeTeam(e.target.value)} className="bg-gray-700 p-2 rounded w-full" required /> <input type="text" placeholder="Opponent Team Name" value={localOpponentTeam} onChange={(e) => setLocalOpponentTeam(e.target.value)} className="bg-gray-700 p-2 rounded w-full" required /> </div> <input type="text" placeholder="Match Name (e.g., vs. Rival High)" value={localMatchName} onChange={(e) => setLocalMatchName(e.target.value)} className="bg-gray-700 p-2 rounded w-full mb-4" /> <div className="space-y-3 max-h-60 overflow-y-auto pr-2">{localRoster.map((p, i) => (<div key={i} className="flex items-center space-x-2"><input type="number" placeholder="#" value={p.number} min="0" onChange={(e) => updatePlayer(i, 'number', e.target.value)} className="bg-gray-700 p-2 rounded w-20 text-center" required /><input type="text" placeholder="Player Name" value={p.name} onChange={(e) => updatePlayer(i, 'name', e.target.value)} className="bg-gray-700 p-2 rounded w-full" required /></div>))}</div> <button type="button" onClick={addPlayer} className="mt-4 w-full bg-gray-600 hover:bg-gray-500 p-2 rounded">Add Player</button> <button type="submit" className="mt-2 w-full bg-cyan-600 hover:bg-cyan-500 p-2 rounded font-bold">Save & Start Match</button> </form>); };
